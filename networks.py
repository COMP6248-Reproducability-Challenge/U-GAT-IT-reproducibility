...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=64, img_size=256, light=False):
        assert (n_blocks >= 0)
        super().__init__()
        ...
        DownBlock.extend([
            ...
            nn.Conv2d(input_nc, ngf, kernel_size=7, stride=1, bias=False),
        ...
        # Down_Sampling Bottleneck
        mult = 2 ** n_downsampling
        for i in range(n_blocks):
            DownBlock.extend([ResnetBlock(ngf * mult, use_bias=False)])
        ...
        for i in range(n_blocks):
            name = 'UpBlock1_' + str(i + 1)
            self.name = ResnetAdaILNBlock(ngf * mult, use_bias=False)
    ...
    def forward(self, input):
        ...
        gap = adaptive_avg_pool2d(x, 1)
        gap_logit = self.gap_fc(gap.view(x.shape[0], -1))
        gap_weight = list(self.gap_fc.parameters())[0]
        gap = x * gap_weight.unsqueeze(2).unsqueeze(3)

        # Adaptive max pooling
        gmp = adaptive_max_pool2d(x, 1)
